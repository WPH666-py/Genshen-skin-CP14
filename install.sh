#!/usr/bin/env bash
# 原神 CP 壁纸套件 14 · 神里绫华 x 优菈 x 甘雨 —— 安装器 (macOS / Linux)
set -e
cd "$(dirname "$0")"
...
if "$PY" -c "import genshen_skin_cp14" >/dev/null 2>&1; then
  echo "[1/2] 已检测到 genshen-skin-cp14"
  RUN=("$PY" -m genshen_skin_cp14.engine.autoinstall)
else
  echo "[1/2] 以仓库源码方式运行(无需预先 pip 安装)"
  RUN=(env PYTHONPATH="$(pwd)/src" "$PY" -m genshen_skin_cp14.engine.autoinstall)
fi
