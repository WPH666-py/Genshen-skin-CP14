// 原神 CP 壁纸套件 14 — 神里绫华 × 优菈 × 甘雨 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP14.git';
const EXT_ID = 'wp666.genshen-skin-cp14';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp14').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp14').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP14'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP14') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp14', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp14.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp14.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp14' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP14') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp14.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(神里绫华 × 优菈 × 甘雨 双人合影), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp14_trio.py 的 MODES 一致。
const CARDS = [
  { mode: 'single1', t: '氷元素学科 · 卡片', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '氷元素学科 · 满屏', img: 'thumb-cover1.png', d: 'cover 铺满整屏' },
  { mode: 'showall1', t: '氷元素学科 · 完整', img: 'thumb-showall1.png', d: '不裁切, 两侧留边' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件 14 · 神里绫华 × 优菈 × 甘雨 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp14.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp14.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp14.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp14.setRandom', () => runCli(['random'], '已随机换一种!'));
  reg('genshencp14.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp14.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp14.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp14.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP14 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp14.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp14')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp14.gallery', new GalleryProvider(), {
