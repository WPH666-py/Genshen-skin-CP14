...
def pkg_root():
    """包目录 .../genshen_skin_cp14(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp14.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eaf1f8;
  --bg-layer-3: #d6e3f0;
  --label-primary: #1b2836;
  --label-secondary: #45586c;
  --label-tertiary: #78899c;
  --brand-primary: #7ba7cc;
  --accent: #7ba7cc;
  --primary: #7ba7cc;
  --interactive-bg-hover-solid: #dfeaf5;
  --interactive-bg-active: #c6dbeb;
  --border-l1: #dbe7f2;
  --border-l2: #c2d5e6;
  --border: #c2d5e6;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP14</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP14", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP14", mascot.get("dark")),
    }
