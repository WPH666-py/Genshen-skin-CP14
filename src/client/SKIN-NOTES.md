...
本目录的 `genshen-cp14.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp14.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp14 deepking          # 导出 ~/.genshen-cp14/deepking/*.skin.json
  ```
...
```bash
genshen-cp14 deepking --check    # 应输出「通过」
genshen-cp14 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp14-light.jpg` —— 取左侧双人(甘雨+优菈), 配浅色界面
- `mascot-cp14-dark.jpg` —— 取右侧双人(优菈+绫华), 配深色界面
